...
export default function StructuralLiquidityPanel() {
  ...
  return (
    <div style={{ fontFamily: FONT }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <span style={{ color: COLORS.amber, fontSize: 14, letterSpacing: 1 }}>STRUCTURAL LIQUIDITY — BIS TOTAL CREDIT</span>
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, padding: '12px 8px', marginBottom: 12 }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <ComposedChart data={chartData} margin={{ top: 5, right: 20, bottom: 5, left: 10 }}>
              ...
              <XAxis dataKey="date" tick={{ fill: COLORS.textMuted, fontSize: 10, fontFamily: FONT }}
                tickFormatter={d => d?.slice(0, 7)} interval="preserveStartEnd" />
              <YAxis tick={{ fill: COLORS.textMuted, fontSize: 10, fontFamily: FONT }}
                tickFormatter={v => `${(v / 1000).toFixed(0)}T`} />
              <Tooltip content={<CustomTooltip />} />
              <Area type="monotone" dataKey="All reporting countries" fill={COLORS.amber} fillOpacity={0.1}
        ...
        <div style={{ background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, padding: '12px 8px' }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <ComposedChart data={data.diffusion.slice(-120)} margin={{ top: 5, right: 20, bottom: 5, left: 10 }}>
              ...
              <Line type="monotone" dataKey="diffusion_weighted" stroke={COLORS.amber}
                strokeWidth={2} dot={false} name="GDP-Weighted" />
              <Line type="monotone" dataKey="diffusion" stroke={COLORS.cyan}
